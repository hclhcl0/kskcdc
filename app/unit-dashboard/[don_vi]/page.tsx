...
export default async function UnitDashboardPage({ params }: { params: { don_vi: string } }) {
  const session = await auth();
  const role = (session?.user as { role?: string })?.role;

  if (role !== 'admin' && role !== 'admin_cdc') redirect('/login');

  const decodedUnit = decodeURIComponent(params.don_vi);

  // Lấy dữ liệu báo cáo của đơn vị
  const reports = await prisma.healthReport.findMany({
    where: { don_vi: decodedUnit },
  ...
  const myProgress = progressData.units.find(u => u.don_vi === decodedUnit);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <Link href="/accounts" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-blue-600 mb-4 transition-colors">
          <ArrowLeft className="w-4 h-4 mr-1" /> Quay lại danh sách tài khoản
        </Link>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
          <Activity className="w-7 h-7 text-blue-600" />
          Dashboard Lũy kế Đơn vị
        </h1>
        <p className="text-slate-500 text-sm mt-1">
          Theo dõi tiến độ thực hiện khám sức khỏe của đơn vị <strong>{decodedUnit}</strong>
        </p>
      </div>
